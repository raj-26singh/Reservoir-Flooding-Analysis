...
# Potha
    ...
    #st.dataframe(pd.DataFrame({"df/dS":dfw_dSw_polymer,"delf/delS":delfw_delSw_polymer}))
    ...
    '''for ite in range(len(diff)):
        if diff[ite]'''
    reff = ceil(up)
    min_index = reff + np.argmin(diff[reff:(len(Sw)-reff)])
    Wid_BT_p = 1.0/dfw_dSw_polymer[min_index]
 
    vel_BT = 1.0/Wid_BT_p
    
    min_index_w = min_index + np.argmin(abs(delfw_delSw_water[min_index:]-np.array([vel_BT]*(len(sat)-min_index))))
    SwBT = Sw[min_index]
    ...
    #st.dataframe(pd.DataFrame({"xD":xD,"Sw":Sw_plot}))
    ...
    #st.write("vel_BT",vel_BT)
    #st.write("min index",min_index)
    #st.write(min_index + np.argmin(abs(delfw_delSw_water[min_index:]-np.array([vel_BT]*(len(sat)-min_index)))))
    #st.write(pd.DataFrame({"Sw":sat,"delfw_delSw_water":delfw_delSw_water}))
    
    temp = np.zeros(len(Sw))
    Sw_bank = sat[min_index + np.argmin(abs(delfw_delSw_water[min_index:]-np.array([vel_BT]*(len(sat)-min_index))))]
    fw_bank = fw_water[min_index + np.argmin(abs(delfw_delSw_water[min_index:]-np.array([vel_BT]*(len(sat)-min_index))))]
    ...
    for i in range(len(Sw)-1,0,-1):
        if sat[i]<=SwBT:
            ...
            temp[i] = ((Sw_bank-Swc) +Wid[i]*(1-fw_bank))
        else:
            Wid[i] = 1.0/dfw_dSw_polymer[i]
    for i in range(len(Sw)-1,0,-1):
        if sat[i]<=SwBT:
            Npd[i] = min(Wid[i],temp[i])
        else:
